...
**When adding a case here, assert on rows, not on a loop's counters.** `relayOnce`, `sweepSlaOnce` and `reconcileOnce` all walk every tenant in the database — that is what makes them background loops rather than request handlers — so `result.outboxRequeued`, `paymentsRetried` and `breaches` describe whatever else is in the database, including rows another integration suite left behind. Three assertions here were written as exact counts and were really asserting that no other suite had leaked; they now read the specific rows the case created, which is both stabler and a stronger claim (ADR-055). Totals are fine when the total is the point — see the "relays every tenant" case, which uses `toBeGreaterThanOrEqual` plus `toContain`.
