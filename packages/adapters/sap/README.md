...
- **`SapAdapter`** (`src/contract.ts`) — the canonical API the app consumes: customer master, catalogue/stock/pricing, order simulate+create, deliveries, invoices, AR open items and incoming payments. A handful of reads take **no account** — `getInquiryQueue`, `getBillingRegister`, `getOpenItemsLedger`, `getRebateRegister`, `getCreditBlockedOrders` — because they serve the back-office desks; each is a method of its own rather than an optional `kunnr` on its customer-plane sibling, so a customer read cannot become a tenant-wide one by dropping an argument (ADR-032, ADR-058). Reads return `SapRead<T>` (`data` + `freshness` + `syncedAt`) so every screen can render `SapSyncIndicator` honestly (docs/05 P1); writes return plain results.
...
- **Desk-plane writes** are real transactions with real refusals: `postOutgoingPayment` (F-58) refuses a document that is not a credit and an amount over what is open; `settleRebateAgreement` (VB(7) refuses anything SAP has not released (KONA-BOSTA != C); `releaseCreditBlock` (VKM3) **re-runs the credit check** and returns `released: false` with a reason when the order is still over its limit or the account is blocked in FD32. All three are idempotent on a reference derived from the document. The seeded landscape carries one releasable blocked order and one that cannot be released, plus one agreement released for settlement, so both outcomes are demoable (ADR-059).
- **Desk-plane reads** compose over the same store the customer reads filter: `getOpenItemsLedger` adds each item's owner from the same ownership map `getOpenItems` filters on, and `getCreditBlockedOrders` derives from the stored orders' CMGST, so an order the credit check holds mid-demo turns up in the release queue with nothing re-seeded.
