...
| [`billing/`](./billing) — `@cc/adapter-billing`                   | **Built (B5).** `BillingAdapter` contract + `mock` driver only — the stub docs/07 B5 asks for, consumed by `@cc/service-platform`'s operator console.                                 |
