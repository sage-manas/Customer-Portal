...
| `listCreditBlockedOrders(sap)`                                          | **Desk plane, no KUNNR.** Orders SAP is holding on credit, longest-waiting first — doc 05 §8's release queue, guarded by `credit:release`. Read-only forever: releasing a block is VKM3 and no adapter method writes VBUK-CMGST (ADR-059). |
