...
| [`notification/`](./notification) — `@cc/service-notification`       | **Built (A7).** Bell inbox and the fan-out behind it. Recipients are resolved from the event, never from a caller (ADR-041).                                              |
| [`reconciliation/`](./reconciliation) — `@cc/service-reconciliation` | **Built (B4).** The outbox half of the exception tray — `payment/` owns the payment half, since services may not import each other (ADR-044).                             |
| [`platform/`](./platform) — `@cc/service-platform`                   | **Built (B5).** The operator console's backend: a wholly separate auth realm from every tenant service above (ADR-045), tenant provisioning, composed health/usage reads. |
