...
import { Badge, Money, PageHeader, relativeTime } from "@cc/ui";
...
/**
 * The exception tray (docs/07 B4, docs/DECISIONS.md ADR-044).
 *
 * Two sources, not one table: a payment reconciliation is stuck on
 * (`@cc/service-payment`'s `listPaymentExceptions`) and an outbox row the
 * relay has given up on (`@cc/service-reconciliation`'s
 * `listOutboxExceptions`). Neither package may import the other (CLAUDE.md
 * rule 1, ADR-011, ADR-027), so this page is what sequences both reads —
 * the same role a route handler plays for any flow that spans two services.
 *
 * Most-severe-then-oldest, the same instinct as the ticket workbench:
 * an operator opening this screen is asking "what needs me right now?", not
...
export default async function ExceptionsPage() {
  ...
  if (!hasPermission(session, "exceptions:view")) redirect("/403");

  const now = new Date();
  const [paymentExceptions, outboxExceptions] = await Promise.all([
    listPaymentExceptions(session.tenantId, now),
    listOutboxExceptions(session.tenantId, now),
  ]);
  ...
  return (
    <>
      <PageHeader
        title="Exceptions"
        subtitle="Payments and outbox events reconciliation couldn't resolve on its own. Most of these clear themselves on the next automatic sweep — retry only if you need it sooner."
      />
